namespace DemBones.Randomizer
{
    ...
    public static class Roll
    {
		...
		=> DemBones(DefaultNumberOfDice, DefaultNumberOfSides, reason);

        public static Bones DemBones<TSides>(int numberOfDice, string reason)
            where TSides : IDie, new() => DemBones(numberOfDice, new TSides().Sides, reason);

        public static Bones DemBones<TSides>(string reason)
            where TSides : IDie, new() => DemBones(1, new TSides().Sides, reason);
    }
}